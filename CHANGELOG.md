...
`_AttachmentInputField`'s OpenAPI description still listed the open attachment
registry as "image / gif / video / **voice** / file" — the exact name this
fleet already agreed was the seam defect (`attachments.py`'s own module
docstring calls the `voice`/`audio` divergence out by name, and a test asserts
the two packages share one vocabulary). The serializer's schema description
was the last generated artifact still carrying the stale name, and every
frontend pair generates its attachment types from `docs/schema.json` — so the
typo was reaching client code, not just a docstring. Two more surviving
listings of the builtin type names were fixed alongside it:
`_capabilities.py`'s CTO-facing axis comment and `docs/capabilities.meta.json`'s
curated `ATTACHMENT_TYPES` summary. `docs/schema.json`, `docs/capabilities.json`
and `docs/llms.txt` are regenerated by `make contract`. No behaviour change —
`audio` was already the only name the registry, the CDN and the tests accept.
