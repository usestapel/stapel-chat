...
| Function | `chat.post_system_message` | in `{conversation_id, body, client_msg_id?}` → out `{message_id, seq, conversation_id}` — appends ONE system line (`sender=null`, `kind=system`) on behalf of no user. The only WRITE on this surface, and narrow on purpose: there is no way for a caller to name an author, because a bus-reachable "post as this person" is a forgery tool in a product where the thread is the record of a deal. `body` is a marker key the reader renders (`video.call.ended:188`), never a rendered sentence. An unknown conversation raises `ConversationNotFound` (a `LookupError`) rather than answering a quiet success | `schemas/functions/chat.post_system_message.json` |
